...
# Use the SAME variable name the rest of the codebase reads (see src/llm/client.py
# and src/embeddings/embedder.py, which both call os.getenv("AZURE_OPENAI_API_KEY")).
# The old name "AZURE_OPENAI_KEY" never matched, so this check always printed False
# even when the key was correctly set in .env.
print("KEY LOADED:", os.getenv("AZURE_OPENAI_API_KEY") is not None)
import json
